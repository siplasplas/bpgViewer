//from https://github.com/rupeshs/BPGviewer.git
...
void QBetterPortableGraphics::decodeBpg(){
    ...
    if (!f) {
        qDebug( )<<"Could not open file "<<QString::fromLatin1(_fileName);
        return;
    }
    ...
    QString outFile = QStandardPaths::
            standardLocations(QStandardPaths::TempLocation).first()+"/bpgviewerimg.png";
    qDebug()<<outFile;
    f = fopen(outFile.toLatin1(), "wb");
    if (!f) {
        qDebug( "I/O error\n");
